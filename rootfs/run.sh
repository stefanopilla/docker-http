...
if [ ! -d "/files" ]; then
	echo -e "Error: No host directory mounted to /files\n" >&2
	exit 1
fi
...
if [ -n "$user" ] && [ -n "$pass" ] || [ -n "$auth" ]; then
	# enable auth
	sed -i 's;<Directory /files>;<Directory /foo>;' /etc/apache2/conf-available/z-custom-config.conf
	sed -i 's;<Directory /marker>;<Directory /files>;' /etc/apache2/conf-available/z-custom-config.conf
	echo "enabled"
